---
title: "Halteverbot: Themendaten + AmtsScore-Vorabwert"
toc: true
---

# Halteverbot

Erste Auswertung aus AmtsGuide-Daten plus **AmtsScore-Vorabwert** pro Stadt, linear normalisiert:

- **40% Kosten** (Mindestgebühr)
- **40% Geschwindigkeit** (Bearbeitungstage)
- **20% Online-Verfügbarkeit** (heute überall null. das Thema ist nur offline)
...
```js
html`<p><strong>Daten-Lücken:</strong> ${hvGap.null_cost}/${hvGap.n_cities} Städte ohne Kosten-Angabe in AmtsGuide. Bei diesen geht der Kosten-Anteil nicht in den Wert ein.</p>`
```
...
Inputs.table(bezirke.map(b => ({
  ...
  "Aktualisiert": b.updated,
  "Bearbeitungsstand": b.status,
})), {
  rows: 20,
  format: {
...
Berlin wird oben auf Stadtebene in der Rangliste geführt, nicht nach einzelnen Bezirken. Bezirks-Daten dienen der Detailrecherche.
...
Diese Auswertung ist **AmtsScore-Vorabwert v0**:
...
- Kein Gesamtwert 0-10
- Stattdessen: nackte Daten zu Behörden-Wartezeiten und Gebühren

Die AmtsScore-Methodik (siehe `/methodik`) misst hingegen die digitale Leistung der Stadt-Verwaltungs-Websites selbst. Beide Ebenen sind komplementär:

- **Diese Themendaten** sagen: *Wie funktioniert dieser konkrete Behördengang in dieser Stadt?*
